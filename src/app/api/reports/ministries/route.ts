...
import { ministryReportSchema } from "@/validators";
...
export async function POST(request: Request) {
  try {
    const payload = ministryReportSchema.parse(await request.json());
    await requirePermission(payload.exportFormat === "view" ? "report.view" : "report.export");

    return reportResponse(await reportService.ministries(payload));
  } catch (error) {
