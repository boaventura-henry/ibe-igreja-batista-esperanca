...
import { eventReportSchema } from "@/validators";
...
export async function POST(request: Request) {
  try {
    const payload = eventReportSchema.parse(await request.json());
    await requirePermission(payload.exportFormat === "view" ? "report.view" : "report.export");

    return reportResponse(await reportService.events(payload));
  } catch (error) {
