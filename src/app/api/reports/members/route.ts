...
import { memberReportSchema } from "@/validators";
...
export async function POST(request: Request) {
  try {
    const payload = memberReportSchema.parse(await request.json());
    await requirePermission(payload.exportFormat === "view" ? "report.view" : "report.export");

    return reportResponse(await reportService.members(payload));
  } catch (error) {
