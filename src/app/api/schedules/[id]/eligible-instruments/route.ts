...
import { eligibleScheduleInstrumentsQuerySchema } from "@/validators";
...
export async function GET(request: NextRequest, context: RouteContext) {
  try {
    ...
    const { categoryId } = eligibleScheduleInstrumentsQuerySchema.parse(Object.fromEntries(request.nextUrl.searchParams));
    return apiSuccess(await scheduleInstrumentAssignmentService.listEligible(id, categoryId, authorization));
  } catch (error) {
