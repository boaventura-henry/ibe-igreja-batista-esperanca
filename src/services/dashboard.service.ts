...
function serializePortalSchedule(schedule: { id: string; roles: Array<{ role: ScheduleMemberRole }>; status: ScheduleMemberStatus; instrumentAssignments: Array<{ instrumentCategory: { id: string; name: string } }>; schedule: { id: string; title: string; date: Date; startTime: string | null; endTime: string | null; location: string | null; ministry: { id: string; name: string; color: string } } }): PortalDashboardSchedule {
  return { id: schedule.schedule.id, scheduleMemberId: schedule.id, title: schedule.schedule.title, date: schedule.schedule.date.toISOString(), startTime: schedule.schedule.startTime, endTime: schedule.schedule.endTime, location: schedule.schedule.location, roles: schedule.roles.map((item) => item.role), instrumentAssignment: schedule.instrumentAssignments[0] ?? null, status: schedule.status, ministry: schedule.schedule.ministry };
}
