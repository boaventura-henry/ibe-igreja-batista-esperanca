...
export function DashboardWidgetRenderer({ widget }: { widget: AuthorizedDashboardWidget }) {
  switch (widget.componentKey) {
    ...
    case "FINANCE_MINISTRY_BALANCES":
      return <section className="rounded-md border border-hope-100 bg-white p-5 shadow-sm"><h2 className="text-lg font-bold text-ink-900">{widget.title}</h2><div className="mt-4 divide-y divide-hope-100">{widget.data.balances.length ? widget.data.balances.map((item) => <div key={item.ministryId} className="flex items-center justify-between gap-3 py-3 text-sm"><span className="font-semibold text-ink-700">{item.ministryName}</span><span className={`font-bold ${Number(item.balance) < 0 ? "text-red-700" : "text-emerald-700"}`}>{formatCurrency(item.balance)}</span></div>) : <p className="py-3 text-sm font-semibold text-ink-500">Nenhum ministerio com movimentacao.</p>}</div></section>;
