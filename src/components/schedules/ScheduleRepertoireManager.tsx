...
function SongSelector({ selectedSong, options, blockedSongIds, value, loading, open, onFocus, onSearch, onSelect, onClear, canCreateSong }: { selectedSong: SongOption | null; options: SongOption[]; blockedSongIds: Set<string>; value: string; loading: boolean; open: boolean; onFocus: () => void; onSearch: (value: string) => void; onSelect: (song: SongOption) => void; onClear: () => void; canCreateSong: boolean }) {
  const normalized = normalizeSearch(value);
  const filtered = options.filter((song) => !normalized || normalizeSearch(`${song.title} ${song.artist ?? ""}`).includes(normalized));
  return <div className="relative grid gap-1 text-xs font-bold uppercase tracking-wide text-ink-500 md:col-span-2"><span>Musica</span><div className="flex gap-2"><input aria-label="Buscar musica por titulo ou artista" className={input} value={value} onFocus={onFocus} onChange={(event) => { onSearch(event.target.value); onFocus(); }} placeholder="Busque por titulo ou artista" /><button type="button" disabled={!selectedSong} onClick={onClear} className="rounded-md border border-hope-100 px-3 py-2 text-xs font-bold disabled:opacity-40">Limpar</button></div>{selectedSong ? <p className="normal-case tracking-normal text-hope-700">Selecionada: {selectedSong.title}{selectedSong.artist ? ` - ${selectedSong.artist}` : ""}{!selectedSong.isActive ? " (inativa, mantida para historico)" : ""}</p> : null}{open ? <div className="absolute left-0 right-0 top-full z-10 mt-1 max-h-56 overflow-y-auto rounded-md border border-hope-100 bg-white p-1 shadow-soft">{loading ? <p className="p-3 normal-case tracking-normal text-ink-500">Carregando musicas...</p> : filtered.length ? filtered.map((song) => { const blocked = blockedSongIds.has(song.id); return <button key={song.id} type="button" disabled={!song.isActive || blocked} onClick={() => onSelect(song)} className="block w-full rounded-md px-3 py-2 text-left normal-case tracking-normal hover:bg-hope-50 disabled:cursor-not-allowed disabled:opacity-50">{song.title}{song.artist ? ` - ${song.artist}` : ""}{!song.isActive ? " (inativa)" : blocked ? " (ja adicionada)" : ""}</button>; }) : <div className="grid gap-2 p-3 normal-case tracking-normal text-ink-500"><span>{options.length ? "Nenhuma musica encontrada." : "Nao ha musicas ativas cadastradas."}</span>{!options.length && canCreateSong ? <Link href="/musicas" className="font-bold text-hope-700">Cadastrar musica</Link> : null}</div>}</div> : null}</div>;
}
