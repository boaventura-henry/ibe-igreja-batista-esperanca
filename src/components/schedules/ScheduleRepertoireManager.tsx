...
import { buildScheduleWhatsAppMessage, copyScheduleWhatsAppText } from "@/lib/schedule-whatsapp-copy";
...
export function ScheduleRepertoireManager({ scheduleId, scheduleTitle, canUpdate }: { scheduleId: string; scheduleTitle: string; canUpdate: boolean }) {
  ...
  function handleQuickSongCreated() {
    setSongSuccessMessage("Música cadastrada e selecionada.");
  ...
  async function copy() {
    ...
    if (!data) {
      setMessage("Aguarde o repertorio carregar para copiar a mensagem.");
      return;
    }
    if (!data.songs.length) {
      setMessage("Adicione uma musica ao repertorio antes de copiar a mensagem.");
      return;
    }

    const text = buildScheduleWhatsAppMessage(scheduleTitle, data.songs);
    copyInProgressRef.current = true;
    setIsCopying(true);
    setMessage("");
    try {
      const copied = await copyScheduleWhatsAppText(text);
      setMessage(copied ? "Repertorio copiado para o WhatsApp." : "Nao foi possivel copiar o repertorio. Tente novamente.");
    } finally {
...
function SongSelector({ selectedSong, options, blockedSongIds, value, loading, open, onFocus, onSearch, onSelect, onClear, onClose, onQuickCreateSuccess, canCreateSong }: { selectedSong: SongOption | null; options: SongOption[]; blockedSongIds: Set<string>; value: string; loading: boolean; open: boolean; onFocus: () => void; onSearch: (value: string) => void; onSelect: (song: SongOption) => void; onClear: () => void; onClose: () => void; onQuickCreateSuccess: () => void; canCreateSong: boolean }) {
  ...
  const quickCreateInProgressRef = useRef(false);
  ...
  async function saveQuickCreate(event: FormEvent) {
    ...
    if (quickCreateInProgressRef.current) return;
    quickCreateInProgressRef.current = true;
    setQuickCreateMessage("");
    ...
    try {
      ...
      quickCreateInProgressRef.current = false;
