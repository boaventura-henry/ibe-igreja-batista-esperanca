...
const emptyForm: AccessRoleFormValues = { name: "", description: "", permissions: [], scheduleScope: "MEMBER_MINISTRIES", isActive: true, confirmSystemChange: false, dashboardWidgets: [], dashboardLayout: { ...defaultDashboardLayout } };
...
const scheduleScopeOptions = [
  {
    value: "ALL",
    label: "Todos os ministerios",
    description: "O usuario podera visualizar e administrar escalas de qualquer ministerio."
  },
  {
    value: "MEMBER_MINISTRIES",
    label: "Apenas ministerios do membro",
    description: "O usuario visualizara apenas as escalas pertencentes aos ministerios aos quais seu membro estiver vinculado."
  }
] as const;
...
export function AccessRoleManager() {
  ...
  return <div className="space-y-5">
    ...
    {isFormOpen ? <div className="fixed inset-0 z-40 overflow-y-auto bg-ink-900/45 px-4 py-6"><div className="mx-auto max-w-5xl rounded-md bg-white shadow-soft"><form onSubmit={handleSubmit}><div className="flex items-start justify-between border-b border-hope-100 px-5 py-4"><div><h2 className="text-lg font-bold">{editingRole ? "Editar perfil" : "Novo perfil"}</h2><p className="text-sm text-ink-500">Permissoes, cards e layout por perfil.</p></div><button type="button" onClick={() => setIsFormOpen(false)} className="rounded-md border border-hope-100 px-3 py-2 text-sm font-bold">Fechar</button></div><div className="grid gap-5 p-5">
      ...
      <fieldset className="rounded-md border border-hope-100 p-4"><legend className="px-1 text-sm font-bold">Escopo Administrativo</legend><p className="mt-1 text-sm text-ink-500">Define quais escalas administrativas os usuarios deste perfil poderao visualizar e administrar.</p><div className="mt-4 grid gap-3 md:grid-cols-2">{scheduleScopeOptions.map((option) => <label key={option.value} className={`flex cursor-pointer items-start gap-3 rounded-md border p-3 transition ${form.scheduleScope === option.value ? "border-hope-500 bg-hope-50" : "border-hope-100 bg-white"}`}><input type="radio" name="scheduleScope" value={option.value} checked={form.scheduleScope === option.value} onChange={() => setForm((current) => ({ ...current, scheduleScope: option.value }))} className="mt-1" /><span className="text-sm font-bold text-ink-900">{option.label}</span></label>)}</div><p className="mt-3 rounded-md bg-hope-50 px-3 py-2 text-xs leading-5 text-ink-600" aria-live="polite">{scheduleScopeOptions.find((option) => option.value === form.scheduleScope)?.description}</p></fieldset>
