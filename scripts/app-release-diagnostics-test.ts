...
assert.deepEqual(appReleases.filter((release) => release.status === "UNRELEASED").map((release) => release.version), [], "0: nenhuma nova versao foi aberta");
...
assert.equal(getLatestPublishedRelease(appReleases)?.version, "0.2.7", "1: catalogo identifica 0.2.7 como ultima release publicada");
