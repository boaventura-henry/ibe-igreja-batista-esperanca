...
const unreleasedReleases: AppRelease[] = [
  ...publishedReleases,
  { version: "0.2.5", title: "Futura", summary: "Entrega ainda nao publicada.", type: "PATCH", status: "UNRELEASED", releaseDate: null, highlights: ["Em desenvolvimento"] }
];
...
assert.equal(appReleases.find((release) => release.version === "0.2.5")?.status, "UNRELEASED", "0: 0.2.5 esta em desenvolvimento");
assert.equal(appReleases.find((release) => release.version === "0.2.5")?.releaseDate, null, "0: 0.2.5 ainda nao possui data de publicacao");
...
await assert.rejects(() => markPublishedReleaseAsSeen({ userId: "user-1", version: "0.2.5" }, { releases: unreleasedReleases }), /nao foi publicada/i, "7: versao futura nao publicada e rejeitada");
