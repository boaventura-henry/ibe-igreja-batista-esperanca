...
assert.equal(appReleases.find((release) => release.version === "0.2.10")?.status, "PUBLISHED", "0: 0.2.10 esta publicada");
assert.equal(appReleases.find((release) => release.version === "0.2.10")?.releaseDate, "2026-09-25", "0: 0.2.10 possui a data oficial de publicacao");
...
assert.equal(getLatestPublishedRelease(appReleases)?.version, "0.2.10", "1: 0.2.10 e a ultima release publicada");
