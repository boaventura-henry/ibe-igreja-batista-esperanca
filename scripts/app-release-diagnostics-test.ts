...
assert.equal(APP_VERSION, "0.2.7", "0: versao ativa aponta para a release 0.2.7");
...
assert.equal(appReleases.find((release) => release.version === "0.2.7")?.status, "UNRELEASED", "0: 0.2.7 esta em desenvolvimento");
assert.equal(appReleases.find((release) => release.version === "0.2.7")?.releaseDate, null, "0: 0.2.7 ainda nao possui data de publicacao");
...
assert.deepEqual(appReleases.slice(0, 8).map((release) => release.version), ["0.2.7", "0.2.6", "0.2.5", "0.2.4", "0.2.3", "0.2.2", "0.2.1", "0.2.0"], "0: catalogo mantem a versao ativa antes do historico");


assert.equal(appReleases.find((release) => release.version === "0.2.0")?.status, "PUBLISHED", "1: 0.2.0 permanece publicada");
assert.equal(getLatestPublishedRelease(appReleases)?.version, "0.2.6", "1: catalogo identifica 0.2.6 como ultima release publicada");
const catalogSeen = await markPublishedReleaseAsSeen(
  { userId: "user-1", version: "0.2.6" },
  {
    releases: appReleases,
    update: async (input) => ({ id: input.userId, lastSeenAppVersion: input.version })
  }
);
assert.equal(catalogSeen.version, "0.2.6", "1: release ativa publicada pode ser marcada como vista");
