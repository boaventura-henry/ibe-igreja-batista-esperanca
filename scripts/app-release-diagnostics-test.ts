...
assert.equal(appReleases.find((release) => release.version === "0.2.4")?.status, "UNRELEASED", "0: 0.2.4 esta em desenvolvimento");
assert.equal(appReleases.find((release) => release.version === "0.2.4")?.releaseDate, null, "0: 0.2.4 nao possui data de publicacao");
...
assert.equal(getLatestPublishedRelease(appReleases)?.version, "0.2.3", "1: catalogo preserva 0.2.3 como ultima release publicada durante o desenvolvimento");
