...
assert.equal(getLatestPublishedRelease(appReleases)?.version, "0.2.5", "1: catalogo identifica 0.2.5 como ultima release publicada");
const catalogSeen = await markPublishedReleaseAsSeen(
  { userId: "user-1", version: "0.2.5" },
  {
    releases: appReleases,
    update: async (input) => ({ id: input.userId, lastSeenAppVersion: input.version })
  }
);
assert.equal(catalogSeen.version, "0.2.5", "1: release ativa publicada pode ser marcada como vista");
