...
const unreleasedReleases: AppRelease[] = [
  ...publishedReleases,
  { version: "0.2.4", title: "Futura", summary: "Entrega ainda nao publicada.", type: "PATCH", status: "UNRELEASED", releaseDate: null, highlights: ["Em desenvolvimento"] }
];

async function main() {

assert.equal(APP_VERSION, "0.2.4", "0: versao ativa aponta para a release 0.2.4");
...
assert.equal(appReleases.find((release) => release.version === "0.2.4")?.type, "PATCH", "0: 0.2.4 e uma release PATCH");
...
assert.deepEqual(appReleases.slice(0, 5).map((release) => release.version), ["0.2.4", "0.2.3", "0.2.2", "0.2.1", "0.2.0"], "0: catalogo mantem a versao ativa antes do historico");


assert.equal(appReleases.find((release) => release.version === "0.2.0")?.status, "PUBLISHED", "1: 0.2.0 permanece publicada");
assert.equal(getLatestPublishedRelease(appReleases)?.version, "0.2.4", "1: catalogo identifica 0.2.4 como ultima release publicada");
const catalogSeen = await markPublishedReleaseAsSeen(
  { userId: "user-1", version: "0.2.4" },
  {
    releases: appReleases,
    update: async (input) => ({ id: input.userId, lastSeenAppVersion: input.version })
  }
);
assert.equal(catalogSeen.version, "0.2.4", "1: release ativa publicada pode ser marcada como vista");
...
await assert.rejects(() => markPublishedReleaseAsSeen({ userId: "user-1", version: "0.2.4" }, { releases: unreleasedReleases }), /nao foi publicada/i, "7: versao futura nao publicada e rejeitada");
