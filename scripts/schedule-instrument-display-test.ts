...
import { getScheduleMemberDisplayRoles } from "../src/lib/schedule-member-role";
...
async function main() {
  assert.equal(
    getScheduleMemberDisplayRoles({ roles: [ScheduleMemberRole.INSTRUMENT] }, bassCategory),
    "Baixo",
  ...
  assert.equal(
    getScheduleMemberDisplayRoles({ roles: [ScheduleMemberRole.INSTRUMENT] }),
    "Instrumento",
  ...
  assert.equal(
    getScheduleMemberDisplayRoles({ roles: [ScheduleMemberRole.VOCAL] }, bassCategory),
    "Vocal",
  ...
  for (const file of files.slice(0, 4)) {
    assert.match(file, /getScheduleMemberDisplayRoles/, "Surface must use the shared display helper.");
  }
